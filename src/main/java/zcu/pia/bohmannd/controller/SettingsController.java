...
//    }
	...
	public ModelAndView changePicture(@RequestParam("file") MultipartFile file, @RequestParam("id") Integer id, ModelAndView mv, HttpSession session, RedirectAttributes redirectAttributes) {
		logger.info("Settings - changeSettings Controller");
		if (session.getAttribute("USER") == null || session.getAttribute("USER").equals("")) {
			logger.info("Not logged in");
			mv.setViewName("redirect:/homepage");
			...
			if (!file.isEmpty()) {
				try {
					...
					File serverFile = new File(dir.getAbsolutePath() + File.separator + id + "-profile-" + file.hashCode() + ".png");
